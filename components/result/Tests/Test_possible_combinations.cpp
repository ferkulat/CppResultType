...
struct IsTrueImpl<ReturnType, PipedType, FunctionType, std::enable_if_t<std::is_same<void, typename ReturnType::type>::value, void>>{
  static bool get(PipedType&& pipedType, FunctionType functionType, [[maybe_unused]] ReturnType&& returnType){
        using actual = decltype(std::forward<PipedType>(pipedType).value | functionType.value);
        using expected = typename ReturnType::type;
        return std::is_same<actual, expected>::value;
  }
};
...
TEST_CASE("Piping std::optional<T> into a function which expects Result<T, E>,  wraps the function return type into std::optional")
{
    ...
    REQUIRE(IsTrue( piping(Opt<int>{2}), toFunction([](Res<int>    )->Opt<float>   {return NullOpt                      ;}             ), returns< Opt<float>       >{                } ));
...
SCENARIO("Piping Result<T, E> into a function which expects T,  wraps the function return type into Result.")
{
    GIVEN("a function is of type F(T)->void") {
        WHEN("piping Result<S,E>(S)"){
            THEN("the function gets called and Result<NothingType,E> is returned"){
                bool called = false;
                REQUIRE(IsTrue(piping(Res<int>{1}), toFunction([&called](int) -> void {called=true;}), returns<Res<NothingType> >{NothingType{}}));
                REQUIRE(called);
            }
        }
        WHEN("piping Result<S,E>(E)"){
            THEN("the function gets called and Result<NothingType,E> is returned"){
                bool called = false;
                REQUIRE(IsTrue(piping(Res<int>{Error::NotOdd}), toFunction([&called](int    )->void {called=true;}), returns< Res<NothingType> >{Error::NotOdd}    ));
                REQUIRE_FALSE(called);
            }
        }
    }
    ...
    REQUIRE(IsTrue(piping(Res<int>{2})            , toFunction([](int    )->Opt<float>   {return NullOpt;}                     ), returns< ResOpt<float>    >{Opt<float>{}    } ));
...
TEST_CASE("Piping Result<T, E> into a function which expects std::optional<T>,  wraps the function return type into Result")
{
    ...
    REQUIRE(IsTrue(piping(Res<int>{2})            , toFunction([](Opt<int>    )->Opt<float>   {return NullOpt                ;}             ), returns< ResOpt<float>    >{Opt<float>{}    } ));
...
TEST_CASE("Piping Result<std::optional<T>, E> into a function which expects T,  wraps the function return type into Result")
{
    ...
    REQUIRE(IsTrue(piping(ResOpt<int>{2})            , toFunction([](int    )->Opt<float>   {return NullOpt;}                ), returns< ResOpt<float>       >{Opt<float>{NullOpt}}        ));
    ...
    REQUIRE(IsTrue(piping(ResOpt<int>{Opt<int>{}})   , toFunction([](int    )->Opt<float>   {return NullOpt;}                ), returns< ResOpt<float>       >{Opt<float>{}}                    ));
    ...
    REQUIRE(IsTrue(piping(ResOpt<int>{Error::NotOdd}), toFunction([](int    )->Opt<float>   {return NullOpt;     }                ), returns< ResOpt<float>       >{Error::NotOdd}                   ));
...
SCENARIO("Piping Result<std::optional<T>, E> into a function which expects std::optional<T>,  wraps the function return type into Result")
{
    GIVEN("a function is of type F(Optional<T>)->void") {
        WHEN("piping Result<Optional<S>,E>(S)"){
            THEN("the function gets called and Result<NothingType,E> is returned"){
                bool called = false;
                REQUIRE(IsTrue(piping(ResOpt<int>{1}), toFunction([&called](Opt<int>    )->void         {called=true;}                                              ), returns< Res<NothingType> >{NothingType{}   } ));
                REQUIRE(called);
            }
        }
        WHEN("piping Result<Optional<S>,E>(E)"){
            THEN("the function gets not called and Result<NothingType,E>(E) is returned"){
                bool called = false;
                REQUIRE(IsTrue(piping(ResOpt<int>{Error::NotOdd}), toFunction([&called](Opt<int>    )->void         {called=true;}                                              ), returns< Res<NothingType> >{Error::NotOdd}    ));
                REQUIRE_FALSE(called);
            }
        }
        WHEN("piping Result<Optional<S>,E>(Optional<S>())"){
            THEN("the function gets not called and Result<NothingType,E>(E) is returned"){
                bool called = false;
                REQUIRE(IsTrue(piping(ResOpt<int>{Opt<int>()}), toFunction([&called](Opt<int>    )->void         {called=true;}                                              ), returns< Res<NothingType> >{NothingType{}}    ));
                REQUIRE(called);
            }
        }
    }
